...
class DiffusionModule(nn.Module):
    ...
    def p_sample(self, xt, t):
        ...
        if t > 1:
            # sigma is based on 3.2 section in DDPM paper.
            # x_t_prev += (1 - extract(self.var_scheduler.alphas, t, xt)).sqrt() * z
            z = torch.randn_like(xt)
            sigma = (((1 - extract(self.var_scheduler.alphas_cumprod, t-1, xt))/(1 - extract(self.var_scheduler.alphas_cumprod, t, xt))) * extract(self.var_scheduler.betas, t, xt)).sqrt()
            x_t_prev += sigma * z
        #######################
        return x_t_prev
    ...
    def ddim_p_sample(self, xt, t, t_prev, eta=0.0):
        ...
        x_0_pred = (xt - (1 - alpha_prod_t).sqrt() * eps_theta) / alpha_prod_t.sqrt()
        x_t_dirt = (1 - alpha_prod_t_prev - torch.square(sigma_t)).sqrt() * eps_theta
        x_t_prev = alpha_prod_t_prev.sqrt() * x_0_pred + x_t_dirt + sigma_t * torch.randn_like(xt)
    ...
    def compute_loss(self, x0):
        ...
        eps_theta = self.network(self.q_sample(x0, t, noise=eps), t)
